...
class CpiDAO {
    ...
    companion object {
        ...
        // Read by USER_ID : USER_ID로 데이터를 검색하는 메서드
        fun selectDataByUserIdClgId(context: Context, user_id: Int,clg_id: Int): MutableList<CPIClass> {
            val dbHelper = DBHelper(context)
            val selection = "USER_ID = ? AND CLG_ID =?"
            val args = arrayOf("$user_id", "$clg_id")
            val db = dbHelper.writableDatabase
